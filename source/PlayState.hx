...
class PlayState extends MusicBeatState
{
	...
	*/
	...
	function RandomBG(a:FlxTimer){
		var random = Std.int((Math.random()*11)+1);
		R34Screen.animation.play('bg$random');
		var _:FlxTimer = new FlxTimer().start(0.13, function(_:FlxTimer){
			R34Screen.animation.play('bg0');
		});
		GlitchSwitch = new FlxTimer().start((Math.random()*6)+2, RandomBG);
	}
	...
	override public function create()
	{
		...
		if(Highscore.Dynamix.get("bt") != null){
			bot = bti(Highscore.Dynamix.get("bt")) > 0;
			if(bot)
				botType = (bti(Highscore.Dynamix.get("bt")) - 1);
			else
				botType = 0;
		}
		if(Highscore.Dynamix.get("bp") != null)
			Bprec = bti(Highscore.Dynamix.get("bp"));
		...
		{
			ip = Highscore.Dynamix.get("prec");
			if(Highscore.Dynamix.get("hell") != null)
			{
				hell = ip && Highscore.Dynamix.get("hell");
			}
			if(ip)
			{
				if(!Bullshit)
		...
		if(SONG.song.toLowerCase() == 'fool-34' || SONG.song.toLowerCase() == 'vibribbon-reds' || SONG.song.toLowerCase() == 'meatball'){
			curStage = "r34room";
		...
		else if(SONG.song.toLowerCase() == 'nick-a-pocalypse')
		{
			var bg:FlxSprite = new FlxSprite(0, 100).loadGraphic('assets/images/nickroomcorupt.png');
			bg.active = false;
			add(bg);
			bg.scale.x *=1.5;
			bg.scale.y *=1.5;
		...
		else if(SONG.song.toLowerCase() == 'glitched-out')
		{
			var bg:FlxSprite = new FlxSprite(0, 100).loadGraphic('assets/images/nickroomholyshit.png', true, 311, 161);
			for(i in 0...11)
				bg.animation.add('bg$i', [i], 0, true);
			bg.active = false;
			add(bg);
			bg.scale.x *= 6;
			bg.scale.y *= 6;
			bg.animation.play('bg0');
			R34Screen = bg;
			GlitchSwitch = new FlxTimer().start((Math.random()*6)+2, RandomBG);
		}
		else
        ...
        if (Name == 'nick-a-pocalypse'){
            gfVersion = 'gf-pixel';
            OffsetGF_X = 100; // Very cool, I bet this feature isn't in kade engine
            OffsetGF_Y = 100;
        }
            

        gf = new Character(400+OffsetGF_X, 130+OffsetGF_Y, gfVersion);
		...
		var Player1:Int = 0xFF66FF33;
		var Player2:Int = 0xFFFF0000;
		switch(SONG.player2){
			case 'nick':
				Player2 = 0xFFD775FF;
			case 'nick-confuse':
			...
			case 'psycho-nick':
				Player2 = 0xFF9719FF;
			case 'spirit-n':
				Player2 = 0xFF343434; // haha funny pr0n number
			 }
        switch(SONG.player1){
            case 'bf':
				Player1 = 0xFF33BBFF;
		}
		healthBar.createFilledBar(Player2, Player1);
		// healthBar
		add(healthBar);
				...
				default:
					...
					switch (Math.abs(i))
					{
						...
						case 3:
							...
							babyArrow.animation.addByPrefix('pressed', sixfret ? (ip ? 'left press' : 'leftb press') : 'right press', 24, false);
							babyArrow.animation.addByPrefix('confirm',sixfret ? (ip ? 'left confirm' : 'leftb confirm') : 'right confirm', 24, false);
						case 4:
							babyArrow.x += Note.swagWidth * 4;
							if(ninefret){
						...
						case 5:
							...
							if(ninefret){
								...
								babyArrow.animation.addByPrefix('pressed', ip&&ninefret ?  'right press' : 'rightb press', 24, false);
								babyArrow.animation.addByPrefix('confirm', ip&&ninefret ? 'right confirm' : 'rightb confirm', 24, false);
							}
						case 6:
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				daNote.active = daNote.visible = activity;
				daNote.y = downscroll ? 
				...
				if(downscroll)
				{
						...
						&& (!daNote.mustPress || (daNote.wasGoodHit || (daNote.prevNote.wasGoodHit && !daNote.canBeHit))))
					{
						...
						var I = (8 * Deceleration) / ((Conductor.songPosition / (Conductor.stepCrochet * 4)) + Deceleration); // Why
						// trace(I);
						if(daNote.mustPress)
							songScore += Std.int(I);
					}
	...
	function endSong():Void
	{
		...
		if(isStoryMode && SONG.song == 'Nick-A-Pocalypse'){
	...
	private function popUpScore(strumtime:Float):Void
	{
		...
		var seperatedScore:Array<String> = Std.string(combo).split(""); // 999+ note fix
	...
	function noteMiss(direction:Int = 1):Void
	{
		...
		{
			healthj -= 0.04;
			if (combo > 5 && gf.curCharacter != 'gf-laptop')
			{
				gf.playAnim('sad');
			}
			combo = 0;
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			switch (note.noteData)
			{
				...
				case 1 | 5 | 7:
					if((sixfret && note.noteData == 1) || (ninefret && note.noteData == 7))
